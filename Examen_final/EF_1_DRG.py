...
class funcion:
    """
    A class to encapsulate a mathematical function and perform operations
    like tabular, graph, and calculate its numerical derivative.
    """
    ...
    def __init__(self, __lambda__):
        ...
        Args:
            __lambda__ (function): A function, preferably lambda, that defines
    ...
    def __call__(self, x):
        """
        Allows the instance of the class to be 'callable' (can be called as a
        function), evaluating the stored function.
        """
        return self.f(x)

    def tabular(self, a, b, n):
        """
        Creates an interval and evaluates the function at each point and prints
        the results.
        """
        table = PrettyTable()
    ...
    def graficar(self, a, b, n):
        """
        Creates and shows a graph of the function in the interval [a, b].
        """
        x_valores = np.linspace(a, b, n)
    ...
    def derivada(self, x0, h=1e-10):
        """
        Calculates the numerical derivative of the function at the point x0.
        """
        numerador = (
            self(x0 - 2 * h) - 8 * self(x0 - h) + 8 * self(x0 + h) -
            self(x0 + 2 * h)
        )
        denominador = 12 * h
